...
import { X, Search, CreditCard, Calendar, User, BookOpen, Loader2, CheckCircle2, Download, Hash, ArrowRight, Receipt, Users, ShieldCheck } from "lucide-react";
...
export default function PaymentModal({ isOpen, onClose, initialStudentId, onSuccess }: PaymentModalProps) {
    ...
    try {
      ...
      if (studentDoc.exists()) {
        ...
        if (std.enrolledClasses && std.enrolledClasses.length > 0) {
           const classPromises = std.enrolledClasses.map(cid => getDoc(doc(db, "classes", cid)));
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const paymentData = {
        ...
        subject: selectedClasses.length > 1 ? "Multiple Classes" : (selectedClasses[0]?.subject || ""),
      ...
      // Generate PDF
      const finalPayment = { id: docRef.id, ...paymentData, createdAt: new Date() } as any;
  ...
  return (
    <div className={`fixed inset-0 z-[120] flex items-center justify-center p-4 sm:p-8 transition-all duration-300 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
       ...
       <div className={`relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden transform transition-all duration-300 ease-out flex flex-col h-[85vh] ${isOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}>
         ...
         <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white z-10 shrink-0">
             <div className="flex items-center gap-5">
                <div className={`w-14 h-14 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-xl font-bold transition-all duration-500 ${loading ? 'animate-pulse bg-primary/10 text-primary' : 'text-primary'}`}>
                    {searching ? <Loader2 className="w-6 h-6 animate-spin" /> : initials}
                </div>
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                         <span className="text-xs font-medium text-slate-500 flex items-center gap-1">
                            <Hash className="w-3.5 h-3.5" /> {selectedStudent?.studentId || "PENDING SELECTION"}
                         </span>
                        ...
                        <section className="space-y-8">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                                <div className="space-y-2">
                                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Student Registry</label>
                                    {!initialStudentId ? (
                                        ...
                                        <div className="px-5 py-4 bg-primary/5 border border-primary/10 rounded-xl flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-xs font-bold text-slate-800">{selectedStudent?.name || 'Loading...'}</p>
                                                    <p className="text-[10px] text-slate-400">Locked ID: {selectedStudent?.studentId}</p>
                                                </div>
                                            </div>
                                            <span className="text-[9px] font-black uppercase bg-white px-2 py-0.5 rounded border border-primary/10 text-primary tracking-widest">Verified</span>
                                        </div>
                                    )}
                                </div>

                                <div className="space-y-2">
                                     <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Academic Class Filter</label>
                                     <div className="space-y-2">
                                         {searching ? (
                                             ...
                                             >
                                                 <div className="flex items-center gap-3">
                                                     <BookOpen className={`w-4 h-4 ${selectedClassIds.includes(cls.id) ? 'text-primary-light' : 'text-slate-300'}`} />
                                                     <div>
                                                         <h5 className="text-xs font-bold">{cls.name}</h5>
                                                         <p className={`text-[9px] font-bold uppercase tracking-widest ${selectedClassIds.includes(cls.id) ? 'text-slate-400' : 'text-slate-400'}`}>{cls.subject}</p>
                                                     </div>
                                                 </div>
                                                 {selectedClassIds.includes(cls.id) && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                                             </button>
                                         )) : studentId ? (
                                             <div className="p-4 bg-rose-50 border border-rose-100 rounded-xl flex items-center gap-3">
                                                 <X className="w-4 h-4 text-rose-500" />
                                                 <p className="text-[10px] font-bold text-rose-600 uppercase">No active enrollment found</p>
                                             </div>
                                         ) : (
                                             <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-3">
                                                 <ArrowRight className="w-4 h-4 text-slate-300" />
                                                 <p className="text-[10px] font-bold text-slate-400 uppercase">Waiting for selection</p>
                                             </div>
                                         )}
                                     </div>
                                </div>
                    ...
                    <div className="space-y-12">
                        <section className="space-y-8">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                                <div className="space-y-2">
                                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Fee Amount (LKR)</label>
                                    <div className="relative group">
                                    ...
                                    <p className="text-[10px] font-bold text-emerald-600 ml-1 italic tracking-wide">Suggested amount based on class registry</p>
                                </div>

                                <div className="space-y-2">
                                     <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Academic Month</label>
                                     <div className="relative group">
                                        ...
                                        <select 
                                            ...
                                            className="w-full pl-14 pr-10 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold uppercase tracking-[0.1em] outline-none focus:border-primary focus:ring-8 focus:ring-primary/5 transition-all appearance-none"
                                        >
                                            {months.map(m => (
                                ...
                                <div className="col-span-full space-y-3">
                                     <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Payment Channel</label>
                                     <div className="grid grid-cols-3 gap-4">
                                        {['cash', 'card', 'bank'].map((m) => (
                                            ...
                                            >
                                                {m}
                                                {method === m && <div className="absolute top-1 right-2"><CheckCircle2 className="w-3 h-3 text-emerald-400" /></div>}
                                            </button>
                                        ))}
                                     </div>
                                </div>
         ...
         <div className="px-8 py-5 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between bg-slate-50/30 z-10 shrink-0">
            <div className="hidden sm:flex items-center gap-3">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Vault Sync Active • Automated Receipting</p>
            </div>
