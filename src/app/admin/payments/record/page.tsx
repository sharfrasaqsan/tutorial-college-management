...
export default function RecordPaymentPage() {
  ...
  const loadArrearsForStudents = async (list: Student[]) => {
    try {
      const snap = await getDocs(collection(db, "payments"));
      const paidInPrevMonth = new Set<string>();
      ...
      snap.docs.forEach((paymentDoc) => {
        ...
        if (
          ...
          paidInPrevMonth.add(payment.studentId);
        }
      });
      ...
      const arrearsIds = new Set(
        activeStudents
          .filter((student) => {
            ...
            return joinedBeforeOrDuring && !paidInPrevMonth.has(student.id);
          })
          .map((student) => student.id),
      );
      setArrearsStudentIds(arrearsIds);
    } catch (error) {
      console.error("Error loading arrears status:", error);
    ...
    async function loadData() {
      ...
      try {
        const gradeSnap = await getDocs(
          query(collection(db, "grades"), orderBy("name", "asc")),
        );
        setGrades(
          gradeSnap.docs.map((d) => ({ id: d.id, ...d.data() }) as Grade),
        );
      } catch (error) {
  ...
  const handleGradeSelect = async (gradeId: string) => {
    ...
    try {
      ...
      const fetchedClasses = snap.docs.map(
  ...
  const handleClassSelect = async (classId: string) => {
    ...
    try {
      ...
      await loadArrearsForStudents(fetchedStudents);

      // Auto-set amount if class has monthly fee
      const cls = classes.find((c) => c.id === classId);
      if (cls)
        setPaymentData((prev) => ({ ...prev, amount: cls.monthlyFee || 0 }));
    ...
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20 mt-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-5">
          <div
            className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-lg ${step === 4 ? "bg-emerald-600 text-white shadow-emerald-200" : "bg-primary text-white shadow-primary/20"}`}
          >
        ...
        <div className="flex items-center gap-3">
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              <div
                className={`flex flex-col items-center gap-1.5 transition-all duration-300 ${step === node.s ? "scale-110 opacity-100" : "opacity-40"}`}
              >
                <div
                  className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[10px] font-black ${node.active ? "bg-emerald-500 border-emerald-500 text-white" : step === node.s ? "border-primary text-primary" : "border-slate-200 text-slate-300"}`}
                >
              ...
              {i < 3 && (
                <div className="w-4 h-[2px] bg-slate-100 mt-[-10px]"></div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {loading ? (
                  [1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className="h-28 rounded-2xl bg-slate-50 animate-pulse border border-slate-100"
                    ></div>
                  ))
                ) : classes.length > 0 ? (
                  classes.map((cls) => (
          ...
          {step === 4 && selectedStudent && (
            <div className="animate-in zoom-in-95 duration-500">
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  // Prevent submission for months before registration
                  if (selectedStudent && selectedStudent.createdAt) {
                    ...
                    const [selectedYear, selectedMonth] = paymentData.month.split("-").map(Number);
                    ...
                    if (selectedYear < joinYear || (selectedYear === joinYear && (selectedMonth - 1) < joinMonth)) {
                      toast.error("Cannot collect payment for months before student registration.");
                      return;
                    }
                  }
                  handleSubmit(e);
              ...
              >
                  ...
                  <div className="flex items-center gap-6 relative z-10">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="text-[10px] font-bold text-emerald-600 tracking-widest uppercase bg-white px-2 py-0.5 rounded-full border border-emerald-100">
                          Verified ID: {selectedStudent.studentId}
                        </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[11px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2">
                      Academic Month
                    </label>
                    <div className="relative group">
                      ...
                      <input
                        ...
                        min={selectedStudent && selectedStudent.createdAt ? 
                        ...
                        className="w-full pl-14 pr-6 py-4 bg-white border-2 border-slate-100 rounded-2xl text-sm font-bold uppercase tracking-widest outline-none focus:border-primary focus:ring-8 focus:ring-primary/5 transition-all"
                        required
                      />
                    </div>
                  </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white sticky top-10 shadow-2xl shadow-slate-200 overflow-hidden">
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur">
                  ...
                  <p className="text-lg font-black uppercase">
                    {formatPaymentMonth(paymentData.month)}
                  </p>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-white/10 space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-[10px] font-black uppercase ${log.color}`}
                    >
                      {log.val}
                    </span>
                  </div>
