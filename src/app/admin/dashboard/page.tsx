...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { title: "Total Teachers", value: stats.totalTeachers, icon: GraduationCap, color: "text-purple-600", bg: "bg-purple-100" },
    { title: "Fees Collected", value: `LKR ${stats.feesCollected.toLocaleString()}`, icon: CreditCard, color: "text-success", bg: "bg-green-100" },
    { title: "Unpaid Fees", value: `${stats.unpaidFeesCount} Students`, icon: AlertTriangle, color: "text-orange-600", bg: "bg-orange-100" },
  ];
          ...
          <div className="p-0 flex-1 overflow-x-auto">
            <table className="w-full text-sm text-left whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100">
                {stats.recentStudents.length > 0 ? stats.recentStudents.map((student: DashboardStudent) => (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-4 flex-1">
            <div className="space-y-4">
              {stats.timetable.length > 0 ? stats.timetable.map((slot: DashboardTimetableSlot) => (
