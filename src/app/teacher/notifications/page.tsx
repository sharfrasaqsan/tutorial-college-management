  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 py-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Faculty Activity Hub</h1>
          <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest flex items-center gap-2">
            <History className="w-3.5 h-3.5" /> Personal Activity Feed • {unreadCount} New Alerts
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 bg-slate-100/50 p-1 rounded-2xl w-fit border border-slate-200/60">
        {[
          ...
          { id: 'unread', label: `New Alerts (${unreadCount})` }
        ].map((tab) => (
          <button
            key={tab.id}
          ...
          {filteredNotifications.length > 0 ? (
            ...
            <div className="py-24 text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 font-medium">You have no active mission alerts or activity logs.</p>
              </div>
            </div>
          )}
        </div>
      ...
      <ConfirmModal 
        ...
        loading={isClearing}
        title="Purge Personal Logs?"
        message="This operation will permanently remove all personal activity records and mission alerts from your terminal. Proceed with authorization?"
        confirmText="Confirm Purge"
        cancelText="Retain Logs"
        variant="danger"
      />
    </div>
  );
