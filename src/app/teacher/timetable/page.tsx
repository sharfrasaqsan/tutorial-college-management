          ...
          return (
            <div key={day} className={`flex flex-col gap-4 p-6 rounded-[2.5rem] border transition-all ${isToday ? 'bg-indigo-600 border-indigo-700 shadow-xl shadow-indigo-200 ring-8 ring-indigo-50' : 'bg-white border-slate-100 shadow-sm group hover:border-indigo-100/50 hover:bg-indigo-50/5'}`}>
               <div className="flex items-center justify-between mb-2">
                    ...
                    {dayClasses.length > 0 && <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-tighter ${isToday ? 'bg-white/20 text-white border border-white/10' : 'bg-indigo-50 text-indigo-700 border border-indigo-100'}`}>{dayClasses.length} Sessions</span>}
               </div>
               ...
               <div className="space-y-3">
                {loading ? (
                    ...
                    <div key={item.id} className={`p-5 rounded-3xl border transition-all group-hover:scale-[1.02] ${isToday ? 'bg-white/10 border-white/10 text-white backdrop-blur-md' : 'bg-white border-slate-100 border-b-4 border-b-slate-50'}`}>
                        <div className="flex flex-col gap-4">
                            <div className="space-y-1">
                                <h4 className="font-bold text-sm tracking-tight leading-none group-hover:text-indigo-600 transition-colors">{item.name}</h4>
                                <div className={`flex items-center gap-1 text-[9px] font-black uppercase tracking-widest ${isToday ? 'text-indigo-100' : 'text-slate-400'}`}>
                                    <BookOpen className="w-2.5 h-2.5" /> {item.subject}
                                </div>
                            </div>
